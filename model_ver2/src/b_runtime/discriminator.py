...
delivered Code2 ``judge`` rule is adapted separately in ``src.model_b`` so the
    ...
    def run_simulation(
        ...
        if scope != "current_store_to_close":
            raise NotImplementedError(
                "This runtime entry point supports current_store_to_close; "
                "the delivered monthly Code2 evaluator remains an offline notebook path"
            )
