...
def _is_execution_eligible(result: Mapping[str, Any]) -> bool:
    """Accept only an actual-B threshold-approved final policy for execution."""
    final_policy = result.get("final_policy")
    return bool(
        result.get("status") == "SUCCESS"
        and result.get("execution_eligible") is True
        and isinstance(final_policy, Mapping)
        and final_policy.get("threshold_pass") is True
    )
...
def optimize_discount_policy(
    ...
    *,
    ...
    previous_discount_rate: Any = None,
) -> dict[str, Any]:
    ...
    if requested_mode != SCOPE_ALIGNED_EXPERIMENTAL:
        raise ValueError(
            "optimize_discount_policy supports only SCOPE_ALIGNED_EXPERIMENTAL so B evaluates "
            "the current store/current-time-to-close threshold"
        )
    runtime_options["discriminator_mode"] = SCOPE_ALIGNED_EXPERIMENTAL
    # The currently valid InitialPolicyLightGBM artifact is NOT_TRAINED until
    # real-B-passed labels exist.  Store operations therefore use the safe
    # CURRENT_POLICY fallback by default; callers may still explicitly set
    # this to False when a trained model is required.
    runtime_options.setdefault("allow_initial_policy_fallback", True)
    ...
    if previous_discount_rate is not None:
        request["previous_discount_rate"] = previous_discount_rate
    ...
    if rolling_enabled:
        ...
        return run_rolling_replan(request, output_path, ledger_path=ledger_path)
...
def optimize_all_store_policies(
    ...
    *,
    ...
    execution_eligible_stores: list[str] = []
    ...
    for index, store_id in enumerate(requested_stores):
        ...
        if _is_execution_eligible(result):
            combined_long.extend(result["final_policy"]["policy_long"])
            execution_eligible_stores.append(store_id)
    ...
    successful = execution_eligible_stores
    response = {
        "schema_version": "1.0",
