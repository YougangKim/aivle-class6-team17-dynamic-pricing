...
def run_policy_discriminator(
    ...
    """Run the scope-aligned operational policy judge."""
    validated, matrix, policy_hash, iteration, session = _policy_context(request, policy)
...
def _build_b_store_state(root, request, mappings):
    ...
    product = (
        pd.read_csv(data_dir / "product.csv")
    ...
    if cells:
        ...
        inventory = pd.read_csv(data_dir / "inventory.csv", parse_dates=["current_date"])
        current = inventory[
    ...
    schedule = resolve_store_schedule(root, store_id, local_timestamp, data_dir=data_dir)
    state = {
        "store_id": store_id,
