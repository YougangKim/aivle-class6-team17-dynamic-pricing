...
class RuntimeStateBuilder:
    def __init__(self, project_root: str | Path, mappings: PolicyMappings) -> None:
        self.root = Path(project_root)
        self.mappings = mappings
        self.product = pd.read_csv(self.root / "data" / "product.csv").set_index("product_id").loc[list(mappings.product_ids)].reset_index()
        self.store = pd.read_csv(self.root / "data" / "store.csv").set_index("store_id")
        self.calendar = pd.read_csv(self.root / "data" / "calendar.csv", parse_dates=["date"]).set_index("date")
        self.store_calendar = pd.read_csv(self.root / "data" / "store_calendar.csv", parse_dates=["date"])
        self.visitor_profile = pd.read_csv(self.root / "data" / "store_visitor_profile.csv")
    ...
    def _empty_table(self, store_id: str, timestamp: pd.Timestamp) -> pd.DataFrame:
        ...
        schedule = resolve_store_schedule(self.root, store_id, local_timestamp)
    ...
    def _from_project_snapshot(self, table: pd.DataFrame, store_id: str, timestamp: pd.Timestamp) -> pd.DataFrame:
        ...
        local_timestamp = timestamp.tz_localize(None) if timestamp.tzinfo is not None else timestamp
        inv = pd.read_csv(self.root / "data" / "inventory.csv", parse_dates=["current_date"])
