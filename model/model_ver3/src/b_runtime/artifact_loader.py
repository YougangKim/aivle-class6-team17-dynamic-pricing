...
def load_b_artifacts(
    ...
    data_dir: str | Path,
    *,
    discriminator_params_path: str | Path | None = None,
) -> BArtifactBundle:
    ...
    discriminator_path = Path(discriminator_params_path).resolve() if discriminator_params_path else artifact_path / "params_discriminator.json"
    if not discriminator_path.exists():
        raise BArtifactError(f"Official B params_discriminator.json is missing: {discriminator_path}")
    ...
    optional = (artifact_path / "surrogate_ridge.pkl",)
