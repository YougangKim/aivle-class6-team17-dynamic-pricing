...
class OuterInnerPolicyOptimizer:
    ...
    def run(self, request, state, initial_policy, b_service):
        ...
        initial = round_execution_policy(initial_policy, mask, self.caps, lower_bounds=self.lower_bounds)
        ...
        for policy in warmup:
            ...
            policy = round_execution_policy(policy, mask, self.caps, lower_bounds=self.lower_bounds)
        ...
        for outer in range(1, int(self.options["max_outer_iterations"]) + 1):
            ...
            inner_count += steps
            candidate = round_execution_policy(proposal, mask, self.caps, lower_bounds=self.lower_bounds)
        ...
        while len(policies) < int(self.options.get("minimum_replay_size", 6)) + 3:
            ...
            policies.append(
                project_policy_numpy(
                    anchor + noise, mask, self.caps, lower_bounds=self.lower_bounds
                )
            )
        return policies
