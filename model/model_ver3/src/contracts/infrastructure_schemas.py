...
def build_runtime_request(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    # Optional rolling fields remain outside the stable A/B envelope's
    # required fields, so existing callers remain byte-for-byte compatible.
    for key in ("previous_discount_rate", "rolling_enabled"):
        if key in value:
